...
async function main() {
	...
	try {
		// Create a new file system based wallet for managing identities.
			...
			const userExists = await wallet.get('USR1');
			if (!userExists) {
				console.log('An identity for the user "USR1" does not exist in the wallet');
			...
			await gateway.connect(ccp, { wallet, identity: 'USR1', discovery: { enabled: true, asLocalhost: true } });
...
app.post('/api/queryUser', async function (req, res) {
    try {
        ...
        res.status(200).json({name : 'Test User 1', phNumber : '9999999999', id : 'TUSR1', fa : 'false', ga : 'false'});
		//*/
